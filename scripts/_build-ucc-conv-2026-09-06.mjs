...
function parseBlocks(raw) {
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    let keyPoints = [];
    let topic = examArea.split("-")[0].trim();
    if (kpMatch) {
    ...
    // Fixed regex: tolerates a blank line and a leading "* " before
    // distractorProfile/incorrectFeedback, which the original ed6 script
    // did not account for, silently dropping this data for prior editions.
    const distractors = {};
    const distractorRegex = /❌ Opción ([A-D]):[^\n]*\r?\n\s*\*?\s*distractorProfile:\s*([^\n]+)\r?\n\*?\s*incorrectFeedback:\s*([^\n]+)/g;
    let distMatch;
    ...
    if (Object.keys(distractors).length === 0) {
      console.warn(`Missing distractors PREGUNTA ${num}`);
    }
...
for (const batch of BATCHES) {
  const mdPath = path.join(__dirname, `../bateria_preguntas/ucc_ed7_lote_${batch.lote}.md`);
  ...
  const exportName = `UCC_CONV_2026_09_06_${sStr}_${eStr}_QUESTIONS`;
  const outputFileName = `ucc-conv-2026-09-06-${sStr}-${eStr}-questions.ts`;
...
/**
 * Convocatoria UCC — Edición #7 (6 sep 2026) · Preguntas #${sStr}–#${eStr}.
...
const aggPath = path.join(__dirname, "../src/data/ucc-conv-2026-09-06-questions.ts");
...
const totalGenerated = generatedInfo.reduce((sum, b) => sum + b.count, 0);
console.log(`\nTotal questions generated: ${totalGenerated}/100`);
