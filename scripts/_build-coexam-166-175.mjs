...
const professorPath = path.join(__dirname, "professor-preguntas-11-20.txt");
const outPath = path.join(__dirname, "update-coexam-166-175.mjs");
...
function renderUpdateScript(questions) {
  ...
  return template
    ...
    .replace(
      ...
      "Patched dr-q-166 through dr-q-175 (${questions.length} questions)",
    );
}
