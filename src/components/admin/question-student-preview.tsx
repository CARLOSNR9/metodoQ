...
export function QuestionStudentPreview({
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <div className="rounded-xl border border-slate-200 bg-white/[0.03] p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 text-lg font-bold text-slate-900">{question.topic}</h3>
            {question.examArea ? (
              <p className="mt-1 text-sm text-slate-500">{question.examArea}</p>
            ) : null}
          </div>
        ...
        <dl className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border border-slate-200 bg-black/20 px-3 py-2">
...
function StudyNotePreview({
  ...
  if (!theoryContent) {
    return (
      <div className="rounded-2xl border border-dashed border-white/15 bg-slate-50 p-8 text-center">
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-[#071428] shadow-[0_32px_120px_-24px_rgb(0_209_255/0.25)]">
      <div className="border-b border-slate-200 bg-gradient-to-br from-mq-accent/15 via-transparent to-indigo-500/10 px-5 py-5 sm:px-7">
        <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <h4 className="mt-2 text-xl font-black text-slate-900">{question.topic}</h4>
        {question.examArea ? (
          <p className="mt-1 text-sm text-slate-500">{question.examArea}</p>
        ) : null}
        <p className="mt-3 font-mono text-xs text-slate-500">{question.id}</p>
      </div>
      ...
      <div className="border-t border-slate-200 bg-[#050f22]/95 px-5 py-4 text-center text-xs text-slate-500 sm:px-7">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !active && "border border-slate-200 text-slate-500 hover:text-slate-900",
      )}
    >
      {children}
    </button>
