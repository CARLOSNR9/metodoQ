  ...
  if (isCheckingAuth || profileLoading) {
    return (
      <section className="space-y-6">
        <div className="h-24 animate-pulse rounded-2xl border border-slate-200 bg-white/[0.04]" />
        <div className="h-96 animate-pulse rounded-2xl border border-slate-200 bg-white/[0.04]" />
      </section>
    );
  }
  ...
  if (!isPro) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold text-slate-900">Mis clases en vivo</h1>
  ...
  return (
    <div className="space-y-8 pb-8">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-black text-slate-900 sm:text-4xl">Mis clases</h1>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        <h2 className="text-xl font-bold text-slate-900">Próximas sesiones</h2>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {upcoming.map((cls) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">{cls.duration} minutos</p>
                {cls.courseName ? (
      ...
      {past.length > 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-900">Clases anteriores</h2>
          </div>
          <ul className="mt-6 space-y-3">
            {past.map((cls) => (
              <li
                key={cls.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/5 bg-slate-50 px-4 py-3"
              >
                <div>
                  <p className="font-medium text-slate-900">{cls.title}</p>
                ...
                {cls.recordingLink ? (
                  <a
                    ...
                    className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-900 hover:bg-white/15"
                  >
                    Ver grabación
                  </a>
                ) : (
                  <span className="text-xs text-slate-500">Sin grabación</span>
                )}
              </li>
            ))}
          </ul>
