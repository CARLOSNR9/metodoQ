...
import { Sparkles, ArrowRight, Zap, Brain, Lightbulb, Trophy, CheckCircle2 } from "lucide-react";
...
export function DailyPillCard({ topic = "Medicina Interna" }: DailyPillCardProps) {
  ...
  return (
    <motion.section 
      ...
      className={`group relative overflow-hidden rounded-[2rem] border ${isCompletedToday ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-mq-accent/30 bg-gradient-to-br from-mq-accent/10 via-mq-surface to-transparent'} p-6 sm:p-8 shadow-xl transition-all duration-500`}
    >
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${isCompletedToday ? 'bg-emerald-500/20 text-emerald-400' : 'bg-mq-accent/20 text-mq-accent'} shadow-[0_0_15px_rgba(0,210,255,0.2)]`}>
              {isCompletedToday ? <CheckCircle2 size={22} /> : <Brain size={22} className="animate-pulse" />}
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <Link
            href={isCompletedToday ? "/dashboard/planes" : "/dashboard/entrenar?mode=daily-pill"}
            className={`w-full md:w-auto mq-premium-glow group/btn inline-flex h-14 items-center justify-center gap-3 rounded-2xl ${isCompletedToday ? 'bg-white/10 text-white border border-white/10 hover:bg-white/20' : 'bg-mq-accent text-mq-accent-foreground hover:scale-105'} px-8 text-sm font-black transition-all active:scale-95`}
          >
